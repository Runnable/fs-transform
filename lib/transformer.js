...
Transformer.prototype.replace = function (rule, cb) {
  ...
  this.driver.grep(rule.search, function (err, grepResult) {
    ...
    // 2.0.1 Filter out files that do not exactly match pattern (for multiline)
    files = files.filter(function (file) {
      // TODO This could possibly use a lot of memory and is pretty slow.
      //      Going to need a better way if this breaks. Happens because the
      //      buffer from readFileSync is being toString-ed.
      var path = self.driver.absolutePath(file);
      var contents = fs.readFileSync(path).toString();
      return contents.indexOf(rule.search) !== -1;
    });
    ...
    async.series([
      ...
      function makeReplacements(cb) {
        async.each(files, function (file, replaceCallback) {
          var path = self.driver.absolutePath(file);
          fs.readFile(file, function (err, data) {
            if (err) { return replaceCallback(err); }
            // TODO This could possibly use a lot of memory and is pretty slow.
            //      Going to need a better way if this breaks. Happens because
            //      the buffer from readFileSync is being toString-ed.
            var contents = data.toString();
            while (contents.indexOf(rule.search) !== -1) {
              contents = contents.replace(rule.search, rule.replace);
            }
            fs.writeFile(path, contents, replaceCallback);
          });
        }, cb);
      },
      ...
      function cleanup(cb) {
        async.each(files, function (name, removeCallback) {
          var copyName = name + Transformer.ORIGINAL_POSTFIX;
          self.driver.remove(copyName, removeCallback);
        }, cb);
      }
    ], cb);
  });
