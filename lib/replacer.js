    ...
    return this.getFiles()
      .then(function (files) {
        return Promise.fromCallback(function (cb) {
          let nonBinary = []
          async.each(files, function isBinary (file, isBinaryCb) {
            childProcess.execFile('file', ['--mime-type', file], function (err, data) {
              if (err) { return isBinaryCb(err) }
              let type = data.split(/\s+/)[1]
              if (type !== 'application/octet-stream') {
                nonBinary.push(file)
              }
              isBinaryCb()
            })
          }, function (err) { cb(err, nonBinary) })
        })
      })
      .then(function (files) {
